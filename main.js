...
function detectCandlePatterns(candles, lookback = 10) {
  const markers = [];
  const start = Math.max(candles.length - lookback, 2); // 至少 2 根
  ...
  for (let i = start; i < candles.length; i++) {
    const c0 = candles[i - 2];   // 兩根前
    const c1 = candles[i - 1];   // 前一根
    const c2 = candles[i];       // 當前
    ...
    /* === 1. 吞噬 === */
    const bullishEngulf = isBear(c1) && isBull(c2) &&
      c2.open <= c1.close && c2.close >= c1.open;
    const bearishEngulf = isBull(c1) && isBear(c2) &&
      c2.open >= c1.close && c2.close <= c1.open;

    /* === 2. 平頭 === */
    const flatBottom = isBull(c1) && isBull(c2) &&
      Math.abs(c1.low - c2.low) < range(c2) * 0.1;
    const flatTop = isBear(c1) && isBear(c2) &&
      Math.abs(c1.high - c2.high) < range(c2) * 0.1;

    /* === 3. 星形 === */
    const smallBody = (c) => body(c) < range(c) * 0.2;
    const morningStar = isBear(c0) && smallBody(c1) && isBull(c2) &&
      c2.close > (c0.open + c0.close) / 2;
    const eveningStar = isBull(c0) && smallBody(c1) && isBear(c2) &&
      c2.close < (c0.open + c0.close) / 2;

    /* === 4. 三線 === */
    const threeWhiteSoldiers = isBull(candles[i-3]) && isBull(candles[i-2]) && isBull(candles[i-1]) &&
      candles[i-3].close < candles[i-2].close && candles[i-2].close < candles[i-1].close;
    const threeBlackCrows = isBear(candles[i-3]) && isBear(candles[i-2]) && isBear(candles[i-1]) &&
      candles[i-3].close > candles[i-2].close && candles[i-2].close > candles[i-1].close;

    /* === 5. 二陰一陽 / 二陽一陰 (Three-Inside) === */
    const twoBearOneBull = isBear(c0) && isBear(c1) && isBull(c2);
    const twoBullOneBear = isBull(c0) && isBull(c1) && isBear(c2);

    /* === 6. 母子 & 母子十字 (Harami / Harami-Cross) === */
    const insideBody   = (inner, outer) =>
      inner.open  >= Math.min(outer.open, outer.close) &&
      inner.open  <= Math.max(outer.open, outer.close) &&
      inner.close >= Math.min(outer.open, outer.close) &&
      inner.close <= Math.max(outer.open, outer.close);

    const bullishHarami       = isBear(c1) && isBull(c2) && insideBody(c2, c1);
    const bearishHarami       = isBull(c1) && isBear(c2) && insideBody(c2, c1);

    const bullishHaramiCross  = bullishHarami && body(c2) < range(c2) * 0.1;
    const bearishHaramiCross  = bearishHarami && body(c2) < range(c2) * 0.1;

    /* === 7. Piercing / Dark-Cloud === */
    const midC1 = (c1.open + c1.close) / 2;
    const piercingLine =
      isBear(c1) && isBull(c2) &&
      c2.open < c1.close && c2.close > midC1 && c2.close < c1.open;
    ...
    const lower = Math.min(c2.open, c2.close) - c2.low;
    const isDoji   = body(c2) < range(c2) * 0.1;
    const isHammer = lower > body(c2) * 2 && upper < body(c2);
    const isInvertedHammer = upper > body(c2) * 2 && lower < body(c2);
    const isShootingStar   = upper > body(c2) * 2 && lower < body(c2) && isBear(c2);
    const isGravestone     = upper > body(c2) * 2 && lower < range(c2)*0.1 && isDoji;
    const isDragonfly      = lower > body(c2) * 2 && upper < range(c2)*0.1 && isDoji;
    const isHangingMan     = lower > body(c2) * 2 && upper < body(c2) && isBear(c2);
    ...
    if (bullishHaramiCross) mark("多頭母子十字", "belowBar", "#26a69a");
    else if (bullishHarami) mark("多頭母子", "belowBar", "#26a69a");

    if (bearishHaramiCross) mark("空頭母子十字", "aboveBar", "#ef5350");
    else if (bearishHarami) mark("空頭母子", "aboveBar", "#ef5350");

    if (piercingLine)   mark("實穿線", "belowBar", "#26a69a");
    if (darkCloudCover) mark("烏雲罩頂", "aboveBar", "#ef5350");
    ...
    if (isHangingMan)    mark("吊人", "aboveBar", "#ef5350");
  }
...
function generateAISuggestion(candles, markers) {
  ...
  /* ========= 近 3 根型態 → 判定 ========= */
  const rawPatterns = markers.slice(-3)
    .map(m => m?.text)
    .filter(Boolean);
  ...
  const biasMap = {
    ...
    "墓碑線":"偏空","T字線":"偏多","倒錘線":"偏多",
    "二陰一陽":"偏多","二陽一陰":"偏空","十字星":"觀察",
    "平頭頂部":"偏空","平頭底部":"偏多",

    /* 🆕 多頭反轉（空轉多） */
    "錘子":"偏多","多頭母子":"偏多","多頭母子十字":"偏多","實穿線":"偏多",
    "多頭星形十字":"偏多","晨星十字":"偏多","多頭棄嬰":"偏多",
    "多頭三星":"偏多","多頭遭遇線":"偏多","多頭缺帶":"偏多",
    "獨特三河床":"偏多","多頭起跑":"偏多","內因三日翻紅":"偏多",
    "外側三日上升":"偏多","南方三星":"偏多","閃中乳燕":"偏多",
    "梯底":"偏多","低價配":"偏多","多頭三明治":"偏多",
    "多頭反撲":"偏多","飛鴿歸巢":"偏多",

    /* 🆕 空頭反轉（多轉空） */
    "吊人":"偏空","空頭母子":"偏空","空頭母子十字":"偏空",
    "烏雲罩頂":"偏空","空頭星形十字":"偏空","夜星":"偏空","夜星十字":"偏空",
    "空頭棄嬰":"偏空","空頭三星":"偏空","雙鴉留空":"偏空",
    "空頭遭遇線":"偏空","空頭缺帶":"偏空","大敵當前":"偏空",
    "空頭起跑":"偏空","三胎鴉":"偏空","雙鴉":"偏空",
    "空頭內因三日下跌":"偏空","空頭反撲":"偏空","步步為營":"偏空"
  };
  ...
  /* ========= 輸出字串 ========= */
  let out = `${bias === "觀察" ? "⚠️ 判斷：觀察"
            : bias === "偏多"  ? "📈 判斷：偏多"
            :                    "📉 判斷：偏空"}\n`;

  out += `📌 形態：${rawPatterns.length
            ? `「${rawPatterns.join("、")}」`
            : "無明顯形態"}\n`;
  ...
  if (!rawPatterns.length)
    return out + `⚠️ 未偵測到明確形態，建議觀察即可。`;
  if (!isValidSetup)
    return out + `⚠️ 雖有形態，但動能不足或實體過小，不建議操作。`;
